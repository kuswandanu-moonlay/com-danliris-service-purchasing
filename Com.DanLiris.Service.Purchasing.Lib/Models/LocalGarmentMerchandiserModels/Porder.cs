using System;
using System.Collections.Generic;

namespace Com.DanLiris.Service.Purchasing.Lib.Models.LocalGarmentMerchandiserModels
{
    public partial class Porder
    {
        public Guid IdPo { get; set; }
        public string Ro { get; set; }
        public string Po { get; set; }
        public string Konf { get; set; }
        public DateTime? Tanggal { get; set; }
        public string Nopo { get; set; }
        public string CodeSpl { get; set; }
        public string Series { get; set; }
        public string Cat { get; set; }
        public string Kodeb { get; set; }
        public string Kodec { get; set; }
        public string Ketr { get; set; }
        public string Urut { get; set; }
        public string Kett { get; set; }
        public string Kett2 { get; set; }
        public string Kett3 { get; set; }
        public string Kett4 { get; set; }
        public string Kett5 { get; set; }
        public decimal? Qty { get; set; }
        public string Satb { get; set; }
        public decimal? Harga { get; set; }
        public decimal? HrgBeli { get; set; }
        public string Buyer { get; set; }
        public string Art { get; set; }
        public DateTime? Tempo { get; set; }
        public string Stat { get; set; }
        public string Def { get; set; }
        public decimal? Jumlah { get; set; }
        public DateTime? Tgl { get; set; }
        public DateTime? Delivery { get; set; }
        public string Seksi { get; set; }
        public string Comp { get; set; }
        public string Clr1 { get; set; }
        public string Clr2 { get; set; }
        public string Clr3 { get; set; }
        public string Clr4 { get; set; }
        public string Clr5 { get; set; }
        public string Clr6 { get; set; }
        public string Clr7 { get; set; }
        public string Clr8 { get; set; }
        public string Clr9 { get; set; }
        public string Clr10 { get; set; }
        public string Real { get; set; }
        public string Revise { get; set; }
        public string Term { get; set; }
        public int? Hari { get; set; }
        public string Rcvd { get; set; }
        public string Valas { get; set; }
        public decimal? Rate { get; set; }
        public string Printed { get; set; }
        public DateTime? Shipment { get; set; }
        public string Post { get; set; }
        public string DtlsPo { get; set; }
        public DateTime? TgValid { get; set; }
        public DateTime? TgPrint { get; set; }
        public DateTime? Planed { get; set; }
        public decimal? JmlPersen { get; set; }
        public string KetOb { get; set; }
        public string FlagOb { get; set; }
        public string MataUang { get; set; }
        public DateTime? ShipDate { get; set; }
        public DateTime? PlanDate { get; set; }
        public DateTime? Deadline { get; set; }
        public decimal? Konv { get; set; }
        public string Satk { get; set; }
        public string Userin { get; set; }
        public DateTime? Tglin { get; set; }
        public string Jamin { get; set; }
        public string Usered { get; set; }
        public DateTime? Tgled { get; set; }
        public string Jamed { get; set; }
    }
}
